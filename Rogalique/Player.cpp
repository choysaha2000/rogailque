...
// set paddle
Paddle::Paddle() : speed(SETTINGS.PADDLE_SPEED) {
...
void Paddle::Init(Game &game) {
  shape.setPosition(SETTINGS.SCREEN_WIDTH / 2.f,
                    SETTINGS.SCREEN_HEIGHT - SETTINGS.PADDLE_Y_OFFSET);
}
...
void Paddle::Update(float deltaTime, Game &game, sf::RenderWindow &window) {
  ...
  if (pos.x - SETTINGS.PADDLE_WIDTH / 2.f < 0.f)
    pos.x = SETTINGS.PADDLE_WIDTH / 2.f;
  if (pos.x + SETTINGS.PADDLE_WIDTH / 2.f > SETTINGS.SCREEN_WIDTH)
    pos.x = SETTINGS.SCREEN_WIDTH - SETTINGS.PADDLE_WIDTH / 2.f;
...
// ball
Ball::Ball() : speed(SETTINGS.BALL_SPEED) {
